...
> **优先走程序化管道**（更快、直达官方原文，三市场均支持）：
> ```bash
> python3 tools/filings_fetch.py list {代码} --type annual        # 列出官方披露（年报/季报/10-K/10-Q）
> python3 tools/filings_fetch.py fetch {代码} --type 10-Q --latest # 下载最新一份到 data/filings/
> ```
> 下载后**用语义管道直接消化原文**（HTML/TXT 零依赖，PDF 需 pypdf）：
> ```bash
> python3 tools/filings_parse.py sections {下载的文件}                    # 查看章节结构
> python3 tools/filings_parse.py extract {下载的文件} --section mda      # 抽取管理层讨论与分析
> python3 tools/filings_parse.py extract {下载的文件} --section risk     # 抽取风险因素
> python3 tools/filings_parse.py diff {去年文件} {今年文件} --section risk # 跨年措辞对比（新增/删除风险逐条列出）
> ```
> 精读顺序建议：mda（经营讨论）→ risk 跨年 diff（新增了什么风险）→ 再读报表附注。
> 工具失败（退出码 1）再走下方 WebSearch 途径或用 pdf 解析能力转文本。

使用 Task 工具启动多个后台 Agent **并行**获取以下原始材料：

1. **财报原文**：首选上方 filings_fetch 管道；不可用时从公司IR页面、SEC EDGAR（美股10-K/10-Q）、港交所披露易（港股）、巨潮资讯网（A股）获取
...
```bash
# 收入和净利润交叉验证（至少2个来源，--values 为 JSON 对象：{来源: 数值}）
python3 tools/financial_rigor.py cross-validate \
  --field revenue --values '{"公司财报": 1083, "stockanalysis": 1079}' --unit 亿
...
**Benford 造假初筛（可选，发现≥2个异常信号时建议执行）**：从财报附注/分部数据中提取 ≥ 50 个原始金额，检验首位数字分布：

```bash
python3 tools/financial_rigor.py benford --values '[金额1, 金额2, ...]'
```
不符合 Benford 分布 ≠ 造假，但须在报告“附注中的隐藏信息”一节标注并建议进一步调查。
...
核心指标（营收/净利润）另配趋势图，按 `references/report-visuals.md` 类型1（图为辅、表为准）。
...
将报告写入 `reports/{公司名}/{公司名}-earnings-review-{期间}.md`（目录不存在先创建，遵循 `CLAUDE.md` 报告路径约定），例如 `reports/腾讯/腾讯-earnings-review-2025Q4.md`。

> 路径说明：与 `earnings-team` 的最终文章（`reports/{公司名}/{公司名}-earnings-{期间}.md`）同目录、不同文件名，两者互不覆盖；下游流程（`thesis-tracker` / `exit-review`）从 `reports/{公司名}/` 目录读取时两类报告均可见。
