...
3. **管理层动作**：高管变动、增减持、回购、分红、股权激励（内部人买卖用 `python3 tools/insider_trading.py recent {代码} --days 30` 取数）
...
4. **大 V 观点**（实验性工具，失败时跳过不阻断流程）：可调用 `python3 tools/experimental/xueqiu_scraper.py` 抓段永平等大 V 最近相关发言
   - 段永平 user_id: `1247347556`
   - 命令示例：`python3 tools/experimental/xueqiu_scraper.py --user-id 1247347556 --keywords {公司名},{股票代码} --output /tmp/dyp-{公司名}.md`
   - 仅在该公司是段永平/李录关注标的时调用，否则跳过节省时间
   - ⚠️ 该工具为实验性脚本（依赖 playwright），调用失败时直接跳过本项，用 WebSearch 替代
