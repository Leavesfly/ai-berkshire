---
name: quality-screen
description: 去劣筛选——用 7 条硬性指标快速排除非一流公司。当用户想对标的做快速去劣初筛、用财务硬指标过滤掉不合格公司、判断是否达到一流公司门槛时使用。
type: executable
confirm_level: light
tools_required: [quality_screen.py, ashare_data.py, report_audit.py]
...
> **首选程序化一键打分**（三市场均支持，每家秒级出结果）：
> ```bash
> python3 tools/quality_screen.py 600519 hk00700 usAAPL     # 批量自动取数打分 7 条指标
> ```
> 工具输出中标「需人工补充」的指标（如 A 股的 FCF/利息覆盖）才走下方 WebSearch 补齐；
> 触犯指标的公司由你结合下文豁免规则与银行/保险特例做终判（工具只做硬指标初筛）。
> 工具整体失败则回退下方逐项取数流程。
...
> ```
> 工具取数后仍需按 `skills/financial-data/SKILL.md` 规范用东方财富/巨潮作第二来源交叉验证；工具失败则回退 WebSearch。
> 港股/美股暂无程序化通道，直接按下列清单 WebSearch。
...
**保存报告**（遵循 `CLAUDE.md` 报告路径约定，目录不存在先创建）：
- 个股模式（单家公司）：`reports/{公司名}/{公司名}-screen-{YYYYMMDD}.md`
- 批量模式（多家/行业/市场/主题）：`reports/{行业或主题名}-screen-{YYYYMMDD}.md`

筛选结果是下游流程（`industry-funnel` / `investment-research`）的输入，不落盘则数据链路断裂，禁止只在对话中输出。

**数据抽检（准出流程）**：报告写入后按 [`references/audit-protocol.md`](../../references/audit-protocol.md) 执行抽检，通过方可交付。
