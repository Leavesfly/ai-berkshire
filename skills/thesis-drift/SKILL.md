...
本 Skill 依赖 `thesis-tracker`（`skills/thesis-tracker/SKILL.md`）输出的结构化维度：核心假设清单、红线清单、估值锚点、追踪记录表。没有这些结构时，先补齐基线，再做漂移检测。
...
**官方披露的措辞漂移可程序化检测**（公司自己的措辞变化比研报措辞更硬）：

```bash
# 下载新旧两期年报后，对风险因素/管理层讨论做跨年 diff
python3 tools/filings_fetch.py fetch {代码} --type annual --latest
python3 tools/filings_parse.py diff {上期文件} {本期文件} --section risk
python3 tools/filings_parse.py diff {上期文件} {本期文件} --section mda
```

重点看三类信号：① 新增实质性风险（非律师式模板披露）；② 删掉了曾强调的优势/承诺；③ 口径悔改（同一指标换了定义或不再披露）。命中任一条均作为对应维度的硬证据引用。
...
2. 不要根据记忆或市场印象补造旧论文
3. 引导用户先使用 `thesis-tracker {公司名} 建立论文`（`skills/thesis-tracker/SKILL.md`）建立结构化基线
...
建议：
1. 先执行 thesis-tracker {公司名} 建立论文
2. 下次有新财报或重大事件后，再执行 thesis-drift {公司名} 旧报告 新报告
```
