...
使用 WebSearch 获取当前股价、估值指标（PE/PB/股息率）、最新财报核心数据，用于填写估值锚点。如果已有该公司的 `investment-research` 或 `investment-team` 报告（`reports/{公司名}/` 下），优先从中读取。
...
**任何一条红线触发 → 在报告中用醒目标注，给出明确的行动建议，并建议用户执行 `exit-review` 制定纪律化退出方案。**
...
追踪次数 ≥ 2 次后，报告「本期关键变化」节另配论文里程碑时间线（建仓→历次检查关键事件），按 `references/report-visuals.md` 类型6（图为辅、表为准）。
