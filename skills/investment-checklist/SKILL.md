...
description: 巴菲特价值投资买入前 Checklist——对一只标的执行巴菲特式买入前逐项检查。当用户想在买入前对某公司做价值投资清单核对、快速判断是否符合买入标准时使用。
type: executable
confirm_level: light
...
**评分标准**（★1-5）：
- ★★★★★：四项全部无情绪信号，买入理由能200字内写清，接受停牌5年
- ★★★★☆：无硬情绪信号，但对长期持有的心理准备不足
- ★★★☆☆：存在轻度FOMO或他人影响，但买入逻辑独立完整
- ★★☆☆☆：主要动机来自他人推荐或近期涨幅
- ★☆☆☆☆：纯FOMO驱动，或无法用200字写清买入理由（→视为未通过本关）

**计分口径**：六关均按★评分，★★★及以上计为“过关”，最终结论中的“X/6关”即过关数量。
...
将完整报告写入（遵循 `CLAUDE.md` 报告路径约定，目录不存在先创建）：
- 单公司：`reports/{公司名}/{公司名}-checklist-{YYYYMMDD}.md`
- 多公司对比：`reports/checklist-多公司对比-{YYYYMMDD}.md`

### 第八步：数据抽检（准出流程）

报告写入后按 [`references/audit-protocol.md`](../../references/audit-protocol.md) 执行抽检，通过方可交付。
