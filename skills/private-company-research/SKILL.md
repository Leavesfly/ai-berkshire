...
> **环境降级约定**：若当前环境不提供 Team 协作工具（TeamCreate / TaskCreate / TeamDelete）：有并行子代理能力则在同一条消息中并行启动 6 个子代理（prompt 不变，改为直接返回完整报告）；无子代理能力则单 Agent 顺序完成 6 个任务再进入第六步交叉验证，并在报告开头标注执行模式。
