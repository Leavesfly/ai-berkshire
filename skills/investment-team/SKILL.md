...
4 份报告到齐后、汇总前，启动红队做空研究员对多头论点进行压力测试：

1. **提取多头论点**：从 4 份报告中提取核心看多逻辑（3-5 条）
2. **启动红队 Agent**：按 `skills/red-team/SKILL.md`「嵌入模式」执行，将 4 份报告摘要 + 多头论点传入，要求输出"三条最强做空论据 + 多头论点逐条反驳"（≤800字）
3. **裁判裁决**：team-lead 逐条评估红队论据质量：
   - 红队论据有数据支撑且逻辑成立 → 最终报告必须纳入 Bear Case，且对应维度评分下调
   - 红队论据为纯推演/数据不足 → 标注为"理论风险"，不影响评分
   - 红队未能构建有效论点 → 在报告中注明"多头论点经受住压力测试"
4. **辩论结果嵌入最终报告**：第 5 节"投资论点（Bull vs Bear）"的 Bear 部分必须反映红队攻击结果，不得弱化

> **降级**：无子代理能力时，team-lead 自行扮演红队角色执行精简版五维攻击（聚焦最薄弱的 2 个维度），标注"team-lead 兼任红队"。

### 第八步：汇总最终报告

综合4份分析报告 + 红队辩论结果，输出以下结构的最终报告：
