...
description: 行业投资研究——产业链全景扫描 + 四大师个股分析框架，系统化研究一个行业。当用户想研究某行业的产业链格局与竞争态势、并对行业内公司做投资分析时使用。
type: executable
confirm_level: medium
tools_required: [financial_rigor.py, report_audit.py, chart_gen.py]
...
3. 产业链全景图按 `references/report-visuals.md` 类型3（Mermaid flowchart）绘制，渲染环境不支持时降级为代码块文本箭头图
4. 每个环节至少分析2-3家头部公司
5. 全球公司扫描要尽可能完整（A股/港股/美股/国际）
6. 最终将完整报告写入 `reports/{行业名}-industry-{YYYYMMDD}.md`（遵循 `CLAUDE.md` 报告路径约定）
