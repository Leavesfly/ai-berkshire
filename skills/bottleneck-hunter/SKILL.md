...
> **渐进式披露**：主流程见下文；定时/高频扫描场景的完整流程与报告模板已外置，按需打开 [`references/hourly-scan-mode.md`](references/hourly-scan-mode.md)。
...
> **渐进式披露**：分层拆解框架（Layer 0-4）、AI基础设施拆解示例、其他趋势的拆解方法已外置到 [`references/supply-chain-framework.md`](references/supply-chain-framework.md)，执行时打开该文件按框架逐层拆解。

核心原则：
- **不停留在概念层，拆到物理实体**
- 重点关注 Layer 2-3（alpha 集中区，市场关注度低）
- Layer 1 已被充分定价，alpha 有限

---

## 第三步：瓶颈识别——寻找"咽喉"

对 Layer 2-3 的每个环节，按 [`references/supply-chain-framework.md`](references/supply-chain-framework.md)「四、瓶颈判定6条标准」逐条评估（供给集中度/扩产周期/替代难度/产能利用率/需求增速/客户验证周期），然后按「五、瓶颈评级规则」给出 S/A/B 评级。

瓶颈地图按「六、瓶颈地图输出模板」格式输出。
...
| 标的 | 建议动作 | 理由 |
|------|---------|------|
| A | 执行 `investment-team`（`skills/investment-team/SKILL.md`）深入研究 | S级瓶颈+多重验证 |
...
定时/高频扫描场景（需外部调度器，如 cron / CI）的完整扫描流程、报告输出规则与两套报告模板已外置到 [`references/hourly-scan-mode.md`](references/hourly-scan-mode.md)，触发该模式时打开该文件严格执行。无调度器时手动按主流程第一至七步单次执行即可。
...
1. **报告位置**：
   ...
   - 每小时/每日扫描：`reports/bottleneck-map/{YYYY-MM-DD}/HH-MM-*.md`（每天一个文件夹，文件命名规则见 [`references/hourly-scan-mode.md`](references/hourly-scan-mode.md)「报告输出规则」节）
