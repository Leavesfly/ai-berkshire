...
tools_required: [portfolio_calc.py, financial_rigor.py, ashare_data.py, report_audit.py]
...
**数据抽检（准出流程）**：组合报告写入后按 [`references/audit-protocol.md`](../../references/audit-protocol.md) 执行抽检，通过方可交付。用户提供的成本价/持仓量等私有数据不在抽检范围，仅抽检公开市场数据（股价/估值/财务指标）。
