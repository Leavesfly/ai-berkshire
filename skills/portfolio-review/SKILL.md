...
description: 组合管理——从“研究单个公司”升级到“管理整个投资组合”，审视持仓结构、集中度、预期回报并给出优化建议。当用户想审视或优化自己的投资组合、评估持仓配置与再平衡时使用。
---
...
> **可计算部分先走工具**（集中度 + 价格相关性矩阵 + 加权预期回报，禁止心算）：
> ```bash
> python3 tools/portfolio_calc.py --holdings '[
>   {"name":"腾讯","code":"hk00700","weight":0.30,"expected_return":0.12},
>   {"name":"现金","code":"cash","weight":0.15,"expected_return":0.04}]' --drawdown
> ```
> `--drawdown` 额外输出当前权重下的历史最大回撤与年化波动（压力测试的量化输入：“能否扛住同等跌幅”）。
> 单标的仓位上限参考可用 `python3 tools/financial_rigor.py kelly --win-prob {p} --win {涨幅} --loss {跌幅}`（胜率/盈亏比必须来自 three-scenario 推演，与集中度建议取交集）。
> expected_return 来自第三步的 three-scenario/reverse-dcf 推演；工具输出的相关性只捕捉价格共振，
> 供应链/监管等隐性关联仍按 4.2 定性识别；无网络时加 --no-corr 只算结构。
...
如果组合中有“不如现金”的仓位，或者现金占比过高，建议使用 `industry-research` 或 `investment-checklist`（均位于 `skills/<名称>/SKILL.md`）对感兴趣的行业/公司进行系统筛选，而非在本Skill内直接推荐个股；对“不如现金”的仓位，用 `exit-review` 做纪律化退出审查。
...
**巴菲特的做法**：伯克希尔历史上多次在找不到好机会时让现金占比升至总资产的 20%-30%（执行时可 WebSearch 伯克希尔最新 13F/年报现金仓位作参考）——当找不到好机会时，现金是最好的仓位。
...
```
一、组合概览（持仓表格+持仓占比图，按 references/report-visuals.md 类型2）
二、单仓位体检（每个持仓的健康状态）
三、组合分析
   - 集中度：是否过度分散/集中？
   - 相关性：隐性关联和风险共振
   - 质量×估值定位：各持仓象限图（类型7，降级为二维定位表）
   - 机会成本：排名最低的仓位是否值得持有？
   - 压力测试：极端情景下的回撤预估
四、调仓建议（具体操作+理由）
五、下次审视时间和关注重点
...
**数据抽检（准出流程）**：组合报告写入后执行抽检，通过方可交付：

```bash
# Step 1 — 提取抽检清单（15%随机抽样）
python3 tools/report_audit.py extract --report reports/portfolio-latest.md

# Step 2 — 对清单每项从可靠信源取数（参见 skills/financial-data/SKILL.md）

# Step 3 — 输出准出/打回判决
python3 tools/report_audit.py verdict --results '<填好的JSON>' --report portfolio-latest.md
```

**【准出】** 全部通过 → 交付；**【打回】** 有不通过 → 修正后重审。用户提供的成本价/持仓量等私有数据不在抽检范围，仅抽检公开市场数据（股价/估值/财务指标）。

> **环境降级约定**：若当前环境无并行子代理能力（Task/Agent 工具），第二步改为单 Agent 顺序为每个持仓取数；无网络时仅基于用户提供数据分析并标注局限（与根 SKILL.md「执行环境自检」一致）。
