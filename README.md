...
[![Version](https://img.shields.io/badge/version-1.3.0-green)](pyproject.toml)
...
| **结论** | 模棱两可 | 必须给出"买入/观望/回避"明确判断 + 置信度标注 |
| **纪律** | 无 | 8 条红线一票否决，决策日志可复盘 + 概率校准 |
| **对抗** | 单向生成，容易集体趋同 | 红队做空研究员强制压力测试多头论点 |
...
> 画像定义见 [`references/masters-profiles.md`](references/masters-profiles.md)，跨全部 26 个子流程保持一致。
...
| `investment-team` | 4 角色多 Agent 并行团队研究 + 红蓝军辩论回合 |
| `red-team` | 红队做空研究员：五维攻击构建最强做空论点 |
...
| `morning-brief` | 晨报：四路信号融合（价格+内幕+13F+持仓）+ 优先级排序 |
| `track-record` | 历史决策准确率复盘 + 错误模式归纳 |
| `self-review` | 系统自审：概率校准诊断 + 规则修改建议（越用越聪明） |
...
| `insider_trading.py` | 内部人/大股东买卖信号（美股 Form 4 / A股增减持 / 港股股份变动披露） | 零依赖 |
| `chart_gen.py` | 出版级图表 PNG 生成 | matplotlib |
| `report_audit.py` | 报告数据抽检准出 + 逻辑链审计（logic-chain） | 零依赖 |
...
# 内部人交易信号（三市场）
python3 tools/insider_trading.py recent usAAPL --days 180   # 美股 Form 4
python3 tools/insider_trading.py recent 300750 --days 90    # A股增减持
python3 tools/insider_trading.py recent hk00700             # 港股股份变动披露
...
# 决策日志：概率标注 + 校准统计（Brier 分数）
python3 tools/decision_log.py add --company 腾讯 --code hk00700 \
  --skill investment-research --verdict 买入 --price 480 --probability 75
python3 tools/decision_log.py calibrate

# 逻辑链审计：检查结论是否有证据支撑（[E1] 标注体系）
python3 tools/report_audit.py logic-chain --report reports/腾讯/腾讯-team-20260722.md
...
│                                                     │
│  ④ 报告抽检准出 + 逻辑链审计                        │
│     report_audit.py 数据抽检，研报级通过方可交付      │
│     logic-chain 检查裸奔结论与证据覆盖率             │
│                                                     │
│  ⑤ 决策日志闭环 + 概率校准                          │
│     每个结论落盘（含置信度）→ 现价复盘 → 错误模式   │
│     Brier 分数检验"说 70% 的事实际发生率"           │
│                                                     │
│  ⑥ 红蓝军对抗                                       │
│     红队做空研究员压力测试多头论点，裁判裁决        │
│                                                     │
└─────────────────────────────────────────────────────┘
```
...
│   ├── insider_trading.py      #   内部人交易信号（Form4/增减持/权益披露）
...
详见 [`CLAUDE.md`](CLAUDE.md) — 全部 26 个子流程执行时的最高优先级约束。
